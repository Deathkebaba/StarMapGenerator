...
def run(numberOfStars=5000, starDistance=15, size=200, centerDistance=100, failsafe=40):
    ...
    while len(stars_map.index) < numberOfStars:
        print(
            "\r",
            f"Goal: {numberOfStars} Star: {len(stars_map.index):6d}  Failsafecounter: {failsafeCounter:2d} time: {time.time()-timer1Start:.2f}",
            end="",
        )
